...
from sqlalchemy import (create_engine, Table, Column, Integer,
                        String, Boolean, Sequence, ForeignKey, DateTime)
...
class ItemCategoryAssociation(Base):
    ...
    def __repr__(self):
        return "<Category(item_id=%s, category_id=%s, updated_on=%s)>" % (
            self.items_id, self.categories_id, self.updated_on)
...
class User(Base):
    ...
    def __repr__(self):
        return "<User(id=%s, name=%s, logged_in=%s)>" % (
            self.id, self.name, self.logged_in)
